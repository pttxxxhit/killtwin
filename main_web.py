...
def main(page: ft.Page):
    ...
    page.web_manifest = {
        ...
        "short_name": "AutoTareas",
        "description": "Herramienta para automatizar tareas con archivos",
        "start_url": "/",
        "display": "standalone",
    ...
    def perform_delete_all(e=None):
        ...
        delete_all_btn.update()
    page.snack_bar = ft.SnackBar(ft.Text(result_text.value))
    page.update()
            ...
            for dup, orig in duplicates:
                ...
                item = ft.Container(
                    content=ft.Row([
                        ...
                        ft.IconButton(icon=ft.Icons.DELETE_OUTLINE, tooltip="Eliminar este duplicado", icon_color=ft.Colors.WHITE, bgcolor=ft.Colors.RED_900, on_click=make_delete_fn(dup)),
                    ], alignment=ft.MainAxisAlignment.START, vertical_alignment=ft.CrossAxisAlignment.START),
                    padding=8, border_radius=6, bgcolor=ft.Colors.with_opacity(0.10, ft.Colors.WHITE),
                )
    ...
    duplicate_files_view = ft.Container(
        content=ft.Column([
            ...
            ft.Row([
                ft.ElevatedButton("Seleccionar carpeta", icon=ft.Icons.FOLDER_OPEN_OUTLINED, on_click=lambda _ev: folder_picker.get_directory_path()),
                selected_dir_text,
                delete_all_btn,
    ...
    def run_organize(_ev=None):
        ...
        try:
            ...
            organize_result_text.color = ft.Colors.RED_400
        organize_result_text.update()
    page.snack_bar = ft.SnackBar(ft.Text(organize_result_text.value))
    page.update()
    ...
    organize_files_view = ft.Container(
        content=ft.Column([
            ...
            ft.Row([
                ft.ElevatedButton("Seleccionar carpeta", icon=ft.Icons.FOLDER_OPEN, on_click=lambda _ev: organize_picker.get_directory_path()),
            ...
            ft.Text("Moverá imágenes, videos, documentos, datasets y comprimidos a subcarpetas.", color=ft.Colors.BLUE_200, size=12),
        ], expand=True),
        expand=True,
        bgcolor=ft.Colors.with_opacity(0.25, ft.Colors.BLACK),
        padding=20,
    ...
    width_field = ft.TextField(label="Ancho", value="800", width=100, bgcolor=ft.Colors.with_opacity(0.1, ft.Colors.WHITE), color=ft.Colors.BLUE_200, border_color=ft.Colors.BLUE_200)
    height_field = ft.TextField(label="Alto", value="600", width=100, bgcolor=ft.Colors.with_opacity(0.1, ft.Colors.WHITE), color=ft.Colors.BLUE_200, border_color=ft.Colors.BLUE_200)
    ...
    def run_resize(_):
        ...
        if not output_folder or not os.path.isdir(output_folder):
            ...
            return
        try:
            width_val = width_field.value or "0"
            height_val = height_field.value or "0"
            width = int(str(width_val))
            height = int(str(height_val))
    ...
    resize_files_view = ft.Container(
        content=ft.Column([
            ...
            ft.Row([
                ft.ElevatedButton("Seleccionar carpeta de entrada", icon=ft.Icons.FOLDER_OPEN_OUTLINED, on_click=lambda _: resize_input_picker.get_directory_path()),
                resize_input_text,
            ], alignment=ft.MainAxisAlignment.START, spacing=10),
            ft.Row([
                ft.ElevatedButton("Seleccionar carpeta de salida", icon=ft.Icons.FOLDER_OPEN_OUTLINED, on_click=lambda _: resize_output_picker.get_directory_path()),
                resize_output_text,
            ], alignment=ft.MainAxisAlignment.START, spacing=10),
            ft.Divider(),
            ft.Row([
            ...
            ft.ElevatedButton("Redimensionar imágenes", icon=ft.Icons.PHOTO_SIZE_SELECT_LARGE, on_click=run_resize, bgcolor=ft.Colors.BLUE_800, color=ft.Colors.WHITE),
            ft.Divider(),
            resize_result_text,
            ft.Text("Las imágenes redimensionadas se guardarán con el prefijo 'resized_'", color=ft.Colors.BLUE_200, size=12),
        ], expand=True),
        expand=True,
        bgcolor=ft.Colors.with_opacity(0.25, ft.Colors.BLACK),
        padding=20,
    ...
    rename_base_name = ft.TextField(label="Nombre base", hint_text="Ej: Factura julio", border_color=ft.Colors.BLUE_200, color=ft.Colors.BLUE_200, width=300)
    selected_files_list = ft.ListView(expand=True, spacing=8, padding=0, auto_scroll=False)
    ...
    rename_files_view = ft.Container(
        content=ft.Column([
            ...
            ft.Row([
                ft.ElevatedButton("Seleccionar archivos", icon=ft.Icons.FILE_UPLOAD, on_click=lambda _: file_picker.pick_files(allow_multiple=True)),
                rename_selected_text,
            ], alignment=ft.MainAxisAlignment.START, spacing=10),
            ft.Divider(),
            rename_base_name,
            ft.ElevatedButton("Renombrar archivos", icon=ft.Icons.DRIVE_FILE_RENAME_OUTLINE, on_click=run_rename, bgcolor=ft.Colors.BLUE_800, color=ft.Colors.WHITE),
            rename_result_text,
            ft.Divider(),
            selected_files_list,
        ], expand=True),
    ...
    rail = ft.NavigationRail(
        ...
        group_alignment=-0.9,
        destinations=[
            ft.NavigationRailDestination(icon=ft.Icons.FIND_IN_PAGE_OUTLINED, selected_icon=ft.Icons.FIND_IN_PAGE, label="Duplicados"),
            ft.NavigationRailDestination(icon=ft.Icons.AUTO_AWESOME_MOTION_OUTLINED, selected_icon=ft.Icons.AUTO_AWESOME_MOTION, label="Organizar"),
            ft.NavigationRailDestination(icon=ft.Icons.CROP_OUTLINED, selected_icon=ft.Icons.CROP, label="Redimensionar"),
            ft.NavigationRailDestination(icon=ft.Icons.DRIVE_FILE_RENAME_OUTLINE, selected_icon=ft.Icons.DRIVE_FILE_RENAME_OUTLINE, label="Renombrar"),
        ],
        on_change=change_view,
        bgcolor=ft.Colors.with_opacity(0.45, ft.Colors.GREY_900),
    )

    # (Opcional) Aquí puedes agregar el FilePicker si lo necesitas para la web
    # page.overlay.append(file_picker)
...
# Change the app launch for web deployment
